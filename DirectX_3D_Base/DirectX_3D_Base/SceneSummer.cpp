...
void SceneSummer::Init()
{
	...
	pTexManager->AddTexture(PATH_TEX_BG_B0, BG_B0_TEX_NUM);	//BG
	pTexManager->AddTexture(PATH_TEX_BG_B1, BG_B1_TEX_NUM);
	pTexManager->AddTexture(PATH_TEX_BG_B2, BG_B2_TEX_NUM);
	pTexManager->AddTexture(PATH_TEX_BG_B3, BG_B3_TEX_NUM);
	pTexManager->AddTexture(PATH_TEX_BG_B4, BG_B4_TEX_NUM);
	pTexManager->AddTexture(PATH_TEX_BG_B5, BG_B5_TEX_NUM);
	pTexManager->AddTexture(PATH_TEX_BG_B6, BG_B6_TEX_NUM);
	pTexManager->AddTexture(PATH_TEX_BG_B7, BG_B7_TEX_NUM);

	pTexManager->AddTexture(PATH_TEX_EFFECT_SUMMER, EFFECT_SUMMER_TEX_NUM);	// 種(夏)
	...
	pModelManager->AddModel(PATH_MODEL_SUMMER_GOAL, SUMMER_GOAL_MODEL_NUM);	//ゴール時
	// ステージブロック
	pModelManager->AddModel(PATH_MODEL_SUMMER_BLOCK, SUMMER_BLOCK_MODEL_NUM);
	// ステージブロック(下の方
	pModelManager->AddModel(PATH_MODEL_SUMMER_BLOCK_LOW, SUMMER_BLOCK_LOW_MODEL_NUM);
	// つぼみ
	pModelManager->AddModel(PATH_MODEL_SUMMER_BUD, SUMMER_BUD_MODEL_NUM);
	// 花
	pModelManager->AddModel(PATH_MODEL_SUMMER_FLOWER, SUMMER_FLOWER_MODEL_NUM);
	
	/* エフェクト(エフェクシア)読み込み */
	EffectManager* pEffectManager = EffectManager::GetInstance();
	// マウスエフェクト
	pEffectManager->AddEffect(PATH_CURSOR_EFFECT_SUMMER, CURSOR_EFFECT_SUMMER_NUM);
...
#pragma region	---背景(0
		...
		Draw_BG0->SetTexture(pTexManager->GetTexture(BG_B0_TEX_NUM));		//	テクスチャ
...
#pragma region	---背景(1
		...
		Draw_BG1->SetTexture(pTexManager->GetTexture(BG_B1_TEX_NUM));		//	テクスチャ
...
#pragma region	---背景(2
		...
		Draw_BG2->SetTexture(pTexManager->GetTexture(BG_B2_TEX_NUM));		//	テクスチャ
...
#pragma region	---背景(3
		...
		Draw_BG3->SetTexture(pTexManager->GetTexture(BG_B3_TEX_NUM));		//	テクスチャ
...
#pragma region	---背景(4
		...
		Draw_BG4->SetTexture(pTexManager->GetTexture(BG_B4_TEX_NUM));		//	テクスチャ
...
#pragma region	---背景(5
		...
		Draw_BG5->SetTexture(pTexManager->GetTexture(BG_B5_TEX_NUM));		//	テクスチャ
...
#pragma region	---背景(6
		...
		Draw_BG6->SetTexture(pTexManager->GetTexture(BG_B6_TEX_NUM));		//	テクスチャ
...
#pragma region	---背景(7
	...
	Draw_BG7->SetTexture(pTexManager->GetTexture(BG_B7_TEX_NUM));		//	テクスチャ
...
#pragma endregion
	...
	// テクスチャ設定
	if (SceneGame::GetInstance()->GetSeason() == SUMMER_1)
	{
		DrawSLogo->SetTexture(pTexManager->GetTexture(SUMMER_1_TEX_NUM));	// テクスチャ設定
	}
	else if (SceneGame::GetInstance()->GetSeason() == SUMMER_2)
	{
		DrawSLogo->SetTexture(pTexManager->GetTexture(SUMMER_2_TEX_NUM));	// テクスチャ設定
	}
...
#pragma region	---カーソル
	...
	homing->SetEffectNum(CURSOR_EFFECT_SUMMER_NUM);
...
#pragma endregion
	...
	//--- BGM再生
	CSound::Play(BGM_SUMMER);
}

// 終了処理
void SceneSummer::Uninit()
{
	//	BGM停止
...
// 更新
void SceneSummer::Update()
{
	
}

// 描画
void SceneSummer::Draw(){
}
