...
#define WINDOW_NAME		_T("ふわりーている")			// ウインドウのキャプション名
...
#pragma endregion
	...
	// effekseerの初期化
	g_EffectManager = EffectManager::GetInstance();
	g_EffectManager->Start();
	...
	// サウンドデータの読み込み
	FileManager::GetInstance()->Load(SOUND_DATA_CSV);
	ResourceCSV* CSV = (ResourceCSV*)FileManager::GetInstance()->Get(SOUND_DATA_CSV);
	float bgm = CSV->GetFloat(0, 0);
	float se = CSV->GetFloat(1, 0);
	CSound::SetVolume_BGM(bgm);
	CSound::SetVolume_SE(se);

	// ロード画面のテクスチャ
	TextureManager::GetInstance()->AddTexture(PATH_TEX_LOAD_SILFY, LOAD_SILFY_TEX_NUM);
	//2022/1/31/Load_Tutorial Shimizu Yosuke --------------------------------------------------
	TextureManager::GetInstance()->AddTexture(PATH_TEX_TUTORIAL_UI, TUTORIAL_UI_TEX_NUM);
	TextureManager::GetInstance()->AddTexture(PATH_TEX_STOP_GAME, STOP_GAME_TEX_NUM);
	//-----------------------------------------------------------------------------------------


	// ロード画面のエフェクト
	EffectManager* pEffectManager = EffectManager::GetInstance();
	pEffectManager->AddEffect(PATH_CURSOR_EFFECT, CURSOR_EFFECT_NUM);	// カーソルエフェクト
	pEffectManager->AddEffect(PATH_CURSOR_EFFECT_SPRING, CURSOR_EFFECT_SPRING_NUM);	// カーソルエフェクト(エンディング＆春
	pEffectManager->AddEffect(PATH_CLOVER_GOAL_EFFECT, CLOVER_GOAL_EFFECT_NUM);	// ゴールエフェクト(クローバー)
	pEffectManager->AddEffect(PATH_SIMPLE_GOAL_EFFECT, SIMPLE_GOAL_EFFECT_NUM);	// ゴールエフェクト(シンプル)
	pEffectManager->AddEffect(PATH_GET_ITEM_EFFECT, GET_ITEM_EFFECT_NUM);		// アイテムゲットエフェクト
...
void Uninit()
{
	// サウンドデータ保存
	ResourceCSV* CSV = (ResourceCSV*)FileManager::GetInstance()->Get(SOUND_DATA_CSV);
	CSV->SetFloat(0, 0, CSound::GetVolume_BGM());
	CSV->SetFloat(1, 0, CSound::GetVolume_SE());
	CSV->Save(SOUND_DATA_CSV);						// 保存
...
void Draw()
{
	...
	Text("master : %f", CSound::GetVolume());
	Text("bgm    : %f", CSound::GetVolume_BGM());
	Text("se     : %f", CSound::GetVolume_SE());
	End();																		//	終了
#endif // _DEBUG
...
/**
* @fn		XMVECTOR* CalcScreenToWorld(XMVECTOR* pout, float Sx, float Sy, float fZ, int Screen_w, int Screen_h, XMMATRIX View, XMMATRIX Prj)
* @brief	スクリーン座標をワールド座標に変換
* @param[out]	pout		計算結果
* @param[in]	Sx			スクリーン座標X
* @param[in]	Sy			スクリーン座標Y
* @param[in]	fZ			射影の値(0～1)
* @param[in]	Screen_w	画面の横幅
* @param[in]	Screen_h	画面の高さ
* @param[in]	View		ビュー行列
* @param[in]	Proj		射影行列
* @return 計算結果
*/
XMVECTOR* CalcScreenToWorld(XMVECTOR* pout, float Sx, float Sy, float fZ, int Screen_w, int Screen_h, XMMATRIX View, XMMATRIX Prj)
{
	...
	// ワールド座標の算出
	*pout = DirectX::XMVector3TransformCoord(DirectX::XMVectorSet(Sx, Sy, fZ, 0.0f), tmp);

	return pout;
}

/**
* @fn		XMVECTOR* CalcScreenToWorld(XMVECTOR* pout, float Sx, float Sy, int Screen_w, int Screen_h, XMMATRIX View, XMMATRIX Prj)
...
* @return 計算結果
*/
XMVECTOR* CalcScreenToXY(XMFLOAT3* pout, float Sx, float Sy, int Screen_w, int Screen_h, XMMATRIX View, XMMATRIX Prj)
{
	XMVECTOR nearpos, farpos;

	// 指定したスクリーン座標をカメラに写っている範囲の最小と最大の点を求める
	CalcScreenToWorld(&nearpos, Sx, Sy, 0.0f, Screen_w, Screen_h, View, Prj);
	CalcScreenToWorld(&farpos, Sx, Sy, 1.0f, Screen_w, Screen_h, View, Prj);
	XMVECTOR vecRay = farpos - nearpos;
	XMVECTOR NormalizeVectorRay = vecRay;
	DirectX::XMVector3Normalize(NormalizeVectorRay);
	...
	XMVECTOR LP0 = DirectX::XMVector3Dot((-nearpos), XYnormal);
	XMVECTOR Lray = DirectX::XMVector3Dot(vecRay, XYnormal);
	// 距離の比からXY平面との交点座標を求める
	XMVECTOR result = nearpos + (LP0 / Lray) * vecRay;
...
DirectX::XMFLOAT3 CalcWorldToScreen(DirectX::XMFLOAT3 worldPos, DirectX::XMMATRIX View, DirectX::XMMATRIX Prj)
{
	...
	// ビュー行列変換
	vPos = DirectX::XMVector3TransformCoord(vPos, View);
	// 射影行列変換
	vPos = DirectX::XMVector3TransformCoord(vPos, Prj);
	// ビューポート行列変換
